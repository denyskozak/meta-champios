...
const POST = async (
    ...
    try {
        ...
        const championshipData = mapChampionshipRPC(championship?.data?.content?.fields);

        const teamNicknamesSafe = Array.isArray(teammateNicknames) ? teammateNicknames : [];

        const teamRegistered = championshipData.teams.some(team => team.name === teamName);
        const leadNameRegistered = championshipData.teams.some(team => team.leadNickname === leadNickname || team.leaderAddress === leaderAddress);
        const someTeamNicknamesRegistered = championshipData.teams.some(team => teamNicknamesSafe.some(nickname => team.teammateNicknames.includes(nickname)));
        ...
        if (leadNameRegistered) {
            res.status(400).json({message: 'Leader nickname already registered in other team'});
            return;
        }

        if (someTeamNicknamesRegistered) {
            res.status(400).json({message: 'Some teammate already registered in other team'});
            return;
        }
